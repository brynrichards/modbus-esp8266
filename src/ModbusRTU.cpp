/*
  ...
  This code is licensed under the BSD New License. See LICENSE.txt for more
  info.
*/
#include "ModbusRTU.h"
...
static const uint16_t _auchCRC[] PROGMEM = {
    ...
    0x0041, 0xC181, 0x8180, 0x4040, 0x0000};

uint16_t ModbusRTUTemplate::crc16(uint8_t address, uint8_t* frame,
                                  uint8_t pduLen) {
...
uint16_t ModbusRTUTemplate::send(uint8_t slaveId, TAddress startreg,
                                 cbTransaction cb, uint8_t unit, uint8_t* data,
                                 bool waitResponse) {
  bool result = false;
...
void ModbusRTUTemplate::task() {
  ...
  if (isMaster) {
  ...
  } else {  // For slave wait for whole message to come (unless
            // MODBUSRTU_MAX_READMS reached)
    ...
    while (millis() - t < _t) {  // Wait data whitespace
      ...
      if (millis() - taskStart >
          MODBUSRTU_MAX_READMS) {  // Prevent from task() executed too long
#if defined(ESP32)
        taskEXIT_CRITICAL(&mux);
#endif
        return;
  ...
  if (isMaster && _slaveId == 0) {  // Check if slaveId is set
    for (uint8_t i = 0; i < _len; i++)
      _port->read();  // Skip packet if is not expected
    _len = 0;
    // if (isMaster) cleanup();
    return;
  }
  if (address != _slaveId &&
      (isMaster || address != MODBUSRTU_BROADCAST)) {  // SlaveId Check
    bool skipPacket = true;
    if (isMaster && address == 0 && _len > 0) {
      // Skip a byte
      address = _port->read();
      _len--;
      skipPacket = address != _slaveId;
    }
  ...
  uint16_t frameCrc =
      ((_frame[_len - 2] << 8) | _frame[_len - 1]);  // Last two byts = crc
  ...
  if (isMaster) {
    _reply = EX_SUCCESS;
    if ((_frame[0] & 0x7F) ==
        _sentFrame[0]) {  // Check if function code the same as requested
    ...
    if (_reply != Modbus::REPLY_OFF) rawSend(_slaveId, _frame, _len);
  }
  // Cleanup
  free(_frame);
  _frame = nullptr;
